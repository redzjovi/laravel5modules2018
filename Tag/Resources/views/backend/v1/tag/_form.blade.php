            ...
            @foreach (config('cms.locales') as $locale => $localeName)
                <b-tab>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                            <input class="form-control form-control-sm {{ $errors->has('title_'.$locale) ? 'is-invalid ' : '' }}" id="title_{{ $locale }}" name="title_{{ $locale }}" {{ $loop->first ? 'required' : '' }} type="text" value="{{ old('title_'.$locale, $model->{'title_'.$locale}) }}" />
